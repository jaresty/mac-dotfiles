...
mods presenterm: user.paste("mods 'Transform the input into a valid multi-slide presenterm deck; output raw Markdown only (no code fences). Front matter exactly: \"--- newline title: <use a descriptive title based on the input, never leave blank, do not use colons or disallowed characters> newline author: Generated (or authors: [...] ) newline date: YYYY-MM-DD newline --- newline\"; no other keys. Produce up to 12 slides (no minimum required); each slide MUST start with a Setext header (Title line, next line exactly ---), include content and references, and end with an HTML comment named end_slide on its own line with nothing else, then a blank line; the final slide MAY omit the closing end_slide. Emit directives only as HTML comments on their own line with exact syntax and nothing else on the line: end_slide = \"<!-- end_slide -->\"; pause = \"<!-- pause -->\"; column_layout with weights = \"<!-- column_layout: [7, 3] -->\"; column with index = \"<!-- column: 0 -->\"; reset_layout = \"<!-- reset_layout -->\"; jump_to_middle = \"<!-- jump_to_middle -->\". Use bullets/prose; support mermaid diagrams via code blocks tagged mermaid +render; ALWAYS close every code block with matching triple backticks. Support LaTeX via code blocks tagged latex +render; support D2 via code blocks tagged d2 +render; support executable snippets via fenced code blocks whose info string starts with a language then +exec (optionally +id:<name>) or +exec_replace or +image; only emit \"<!-- snippet_output: name -->\" if a snippet with +id:name exists. Hide code lines with # or /// prefixes per language; other code blocks only if relevant (name the language); images only if valid paths/URLs. Escape all raw < and > characters in slide content to &lt; and &gt; except inside the required HTML comment directives. Mermaid safety: inside mermaid code blocks, do NOT HTML-encode or entity-encode label text; do NOT encode ampersands; do NOT encode parentheses, brackets, or braces; instead, ALWAYS wrap NODE and EDGE LABEL TEXT in ASCII double quotes and escape any double quote inside the label as \\\". Preserve Mermaid grammar and delimiters ([], (), [[]], (()), [/ /]) unmodified. Markdown safety in SLIDE BODY (outside all fenced/inline code and outside HTML directives): HTML-encode tildes unless they form valid strikethrough markup; encode every standalone or path-embedded \"~\" as \"&#126;\" (so \"~/foo\" becomes \"&#126;/foo\"), but keep intentional \"~~text~~\" unchanged; you MAY also encode bare \"*\"->\"&#42;\", \"_\"->\"&#95;\", and \"`\"->\"&#96;\" when not part of valid emphasis/code. Avoid # headers. This instruction itself must contain no literal newline characters. Note: because this string is used in Talon, every literal curly brace in this instruction is doubled (\"{{\" and \"}}\").' > presentation.md")
